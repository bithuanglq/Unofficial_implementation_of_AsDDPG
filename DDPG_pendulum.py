...
MAX_EPISODES = 60          # total number of episodes for training
    ...
    if args.train_or_test:  # train
        ...
        for i in range(MAX_EPISODES):
            ...
            if reward_buffer:
                ...
                plt.cla()
                plt.title('DDPG')
                plt.plot(np.array(range(len(reward_buffer))) * TEST_PER_EPISODES, reward_buffer, label='Episode reward')  # plot the episode vt
        ...
        plt.ioff()
        plt.show()
        # plt.savefig('DDPG_temp_result.png')
        plt.close()
